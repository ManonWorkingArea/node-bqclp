...
export default {
    ...
    computed: {
      mainStyle() {
        const minHeight = (this.configs.siteType === 'asset' && this.session.role === 'admin') ? 'calc(100vh - 285px)' : 'calc(100vh - 165px)';
        return {
          minHeight,
        };
      },
          ...
          <div class="grid grid-cols-3 md:grid-cols-6 gap-2">
            ...
            <div v-for="app in filteredApps" :key="app.href" class="app-container mt-5">
              <template v-if="isSessionDataExists(app)">
                <div class="relative">
                  <div
                    class="app-icon relative flex items-center justify-center w-20 h-20 rounded-lg shadow-md cursor-pointer hover:shadow-lg hover:ring-4 hover:ring-white"
                    @click.stop="app.hasSubmenu ? toggleSubMenu(app) : removeSession(app.session.name)"
                  >
                    <div
                      class="app-icon__inner flex items-center justify-center w-full h-full rounded-lg bg-white shadow-lg"
                      :class="this.configs.siteStyle"
                    >
                      <font-awesome-icon :icon="['fas', app.session.yes.icon]" class="text-white text-3xl" />
                    </div>
                    <span
                      class="notification-count absolute top-0 right-0 bg-red-500 text-white font-bold text-md rounded-full flex items-center justify-center w-6 h-6 shadow-md"
                      v-if="app.counter"
                    >{{ app.counter }}</span>
                  </div>
                </div>
                <transition name="slide-up">
                  <div
                    class="sub-apps fixed inset-0 flex items-center justify-center bg-black bg-opacity-50"
                    :class="{ 'sub-apps--empty': !app.submenu || app.submenu.length === 0 }"
                    v-show="app.showSubMenu"
                  >
                    <div class="sub-apps__content bg-white rounded-sm shadow-md px-3 py-3" ref="subAppsContent">
                      <div class="text-left">
                        <router-link
                          v-for="subApp in app.submenu"
                          :key="subApp.href"
                          :to="subApp.href"
                          class="sub-app flex bg-white items-center justify-start w-full h-12 px-4 py-2 hover:bg-gray-100 cursor-pointer"
                        >
                          <font-awesome-icon :icon="['fas', subApp.icon]" class="text-black text-sm mr-2" />
                          <span class="text-gray-700 text-[18px] font-normal flex-grow-1 hover:font-medium" style="width: 100%;">{{ subApp.name }}</span>
                        </router-link>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="sub-apps__content bg-white rounded-sm shadow-md px-3 py-3" ref="subAppsContent">
                    <div class="grid grid-cols-2 gap-2">
                      <router-link
                        ...
                        :key="subApp.href"
                        :to="subApp.href"
                        class="sub-app flex flex-col items-center justify-center hover:bg-gray-100 cursor-pointer transition-colors duration-200"
                      >
                        <div class="flex flex-col items-center justify-center text-center p-3">
                          <font-awesome-icon :icon="['fas', subApp.icon]" class="text-black text-2xl mb-2" />
                          <span class="text-gray-700 text-base font-medium">{{ subApp.name }}</span>
                        </div>
                      </router-link>
                    </div>
                  </div>
